                                ...
                                int width, int height, int num_frames)
{
    ...
    for(int n = 0; n < num_frames; n++){
        int index = n*width*height*3 +  (y * width + x) * 3;
        int index_gray = n*width*height +  (y * width + x) ;
        for(int c = 0; c < 3; c++){
            rgb[c] = d_input[index+c];
        }
        d_output[index_gray] = (unsigned char) (0.299f * rgb[0] + 0.587f * rgb[1] + 0.114f * rgb[2]);
    }
    ...
    for(int n = 0; n < num_frames; n++){
        for(int y = 0; y < height; y++){
            for(int x = 0; x < width; x++){
                ...
                output[index_gray] = (unsigned char) (0.299f * rgb[0] + 0.587f * rgb[1] + 0.114f * rgb[2]);
            }
        }
    }
...
int main(int argc, char **argv) 
{
    ...
    int size_rgb = num_frames * width * height * 3;
    int size_gray = num_frames * width * height;


    unsigned char* h_output_cpu = (unsigned char*) malloc(size_gray*sizeof(unsigned char));
    unsigned char* h_output_gpu = (unsigned char*) malloc(size_gray*sizeof(unsigned char));

    unsigned char* d_input; // Dichiarazione di un puntatore per la memoria sulla GPU (device)
    cudaMalloc((void**)&d_input, size_rgb*sizeof(unsigned char)); // Allocazione della memoria sulla GPU

    // Copia dei dati dalla memoria dell'host (CPU) alla memoria del device (GPU)
    cudaError_t err = cudaMemcpy(d_input, h_input, size_rgb*sizeof(unsigned char), cudaMemcpyHostToDevice);
    ...
    cudaMalloc((void**)&d_output, size_gray*sizeof(unsigned char)); 
    ...
    if (use_3d) {
        ...
        grid_x = width/blockSize+1;
        grid_y = height/blockSize+1;
        grid_z = num_frames;
        ...
        grayscale_3D<<<grid_size, block_size>>>(d_input, d_output,width,height,num_frames);
        cudaDeviceSynchronize();
        err = cudaMemcpy(h_output_gpu,d_output, size_gray*sizeof(char), cudaMemcpyDeviceToHost);
        if (err != cudaSuccess) {
            // Se c'è un errore, stampa un messaggio di errore e termina il programma
        ...
        printf("\nEsecuzione GPU | 2D loop...\n");

        clock_t gpu_start = clock();
        grayscale_loop<<<grid_size, block_size>>>(d_input, d_output,width,height,num_frames);
        cudaDeviceSynchronize();
        err = cudaMemcpy(h_output_gpu,d_output, size_gray*sizeof(char), cudaMemcpyDeviceToHost);
        if (err != cudaSuccess) {
            // Se c'è un errore, stampa un messaggio di errore e termina il programma
    ...
    printf("\nVerifica correttezza...\n");
    bool correct = verifyResults(h_output_cpu, h_output_gpu, size_gray, "GPU");
